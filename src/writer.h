...
#ifndef MPCD_WRITER_H
#define MPCD_WRITER_H
...
class PosWriter {
    ...
    public:
    PosWriter(H5::H5Location& file, const std::string& name, const unsigned int nbeads) {
...
class VelWriter {
    ...
    public:
    VelWriter(H5::H5Location& file, const std::string& name, const unsigned int nbeads) {
        const hsize_t mem_dims[2] = {nbeads, 3};
        const hsize_t file_dims[3] = {0, nbeads, 3};
        const hsize_t max_dims[3] = {H5S_UNLIMITED, nbeads, 3};
        const hsize_t chunk_dims[3] = {10, nbeads, 3};
...
class ConfigWriter {
    ...
    ConfigWriter(H5::H5Location& file, const std::string& name) {
        ...
        dataset = file.createDataSet(name, H5::PredType::NATIVE_UINT64, file_space, prop);
...
class ConfigCountWriter {
    ...
    public:
    ConfigCountWriter(H5::H5Location& file, const std::string& name, const unsigned int nstates) {
    ...
    void append(const std::vector<uint64_t>& config_count) {
        ...
        file_space.selectHyperslab(H5S_SELECT_SET, count, start);
        dataset.write(config_count.data(), H5::PredType::NATIVE_UINT64, mem_space, file_space);
...
class ConfigProbWriter {
    ...
    public:
    ConfigProbWriter(H5::H5Location& file, const std::string& name, const unsigned int nsteps) {
    ...
    void append(const std::vector<double>& config_prob) {
        ...
        const hsize_t start[2] = {step, 0};
        file_space.selectHyperslab(H5S_SELECT_SET, count, start);
        dataset.write(config_prob.data(), H5::PredType::NATIVE_DOUBLE, mem_space, file_space);
