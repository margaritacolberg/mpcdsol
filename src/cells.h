...
struct Cells {
    // the inner vector stores the indices of the hard spheres
    // and the outer vector stores the indices of the cells the
    // hard spheres are located in
    ...
    Cells(const unsigned int ncell, const double lcell) : cells(ncell * ncell * ncell), ncell(ncell), lcell(lcell), inv_lcell(1 / lcell) {}
};
