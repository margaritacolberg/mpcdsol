...
void init_vel_sol(std::vector<Vec3> &vel, Random &mt, const double temp,
                  const double m) {
...
void delta_tpv(const std::vector<Vec3> &pos, const std::vector<Vec3> &vel,
               ...
               const std::vector<Vec3> &vel_sol, const Box &box,
               const std::vector<double> &times,
               const std::vector<double> &times_sol, unsigned int i,
...
void if_coll(const std::vector<Vec3> &pos, const std::vector<Vec3> &vel,
  ...
  delta_tpv(pos, vel, pos, vel, box, times, times, i, j, t, dx, dy, dz, dvx,
            dvy, dvz);
...
// counters
void if_bead_coll_sol(const std::vector<Vec3> &pos,
                      const std::vector<Vec3> &vel,
                      const std::vector<Vec3> &pos_sol,
                      const std::vector<Vec3> &vel_sol, const double sigma_sb2,
                      const Box &box, const std::vector<uint64_t> &counter,
                      const std::vector<uint64_t> &counter_sol,
                      EventQueue &event_queue, const std::vector<double> &times,
                      const std::vector<double> &times_sol, unsigned int i,
                      unsigned int i_sol, double step_time) {
  double t, dx, dy, dz, dvx, dvy, dvz;
  delta_tpv(pos, vel, pos_sol, vel_sol, box, times, times_sol, i, i_sol, t, dx,
            dy, dz, dvx, dvy, dvz);

  // if a bead elastically collides with a solvent particle,
  if (t_until_inner_coll(dx, dy, dz, dvx, dvy, dvz, sigma_sb2, t) &&
      (t <= step_time)) {
...
void iterate_bead_coll_sol(
    ...
    const std::vector<Vec3> &pos_sol, const std::vector<Vec3> &vel_sol,
    const double sigma_sb2, const Box &box,
    const std::vector<uint64_t> &counter,
    const std::vector<uint64_t> &counter_sol, EventQueue &event_queue,
    const std::vector<double> &times, const std::vector<double> &times_sol,
    const Cells &cells, unsigned int icell, unsigned int i, double step_time) {
  for (unsigned int i_sol : cells.cells[icell]) {
    if_bead_coll_sol(pos, vel, pos_sol, vel_sol, sigma_sb2, box, counter,
                     counter_sol, event_queue, times, times_sol, i, i_sol,
                     step_time);
  }
}

void iterate_sol_coll_bead(
    const std::vector<Vec3> &pos_sol, const std::vector<Vec3> &vel_sol,
    const std::vector<Vec3> &pos, const std::vector<Vec3> &vel,
    const double sigma_sb2, const Box &box,
    const std::vector<uint64_t> &counter_sol,
    const std::vector<uint64_t> &counter, EventQueue &event_queue,
    const std::vector<double> &times_sol, const std::vector<double> &times,
    const Cells &cells, unsigned int icell, unsigned int i_sol,
    double step_time) {
...
void add_events_for_one_bead_coll_sol(
    const std::vector<Vec3> &pos, const std::vector<Vec3> &vel,
    const std::vector<Vec3> &pos_sol, const std::vector<Vec3> &vel_sol,
    const double sigma_sb2, const Box &box,
    const std::vector<uint64_t> &counter,
    const std::vector<uint64_t> &counter_sol, EventQueue &event_queue,
    const std::vector<double> &times, const std::vector<double> &times_sol,
    const Cells &cells_bead, const Cells &cells_sol, unsigned int i,
    double step_time) {
  unsigned int zmin, zmax, ymin, ymax, xmin, xmax;
  ...
  for (unsigned int z = zmin; z <= zmax; z++) {
    for (unsigned int y = ymin; y <= ymax; y++) {
      for (unsigned int x = xmin; x <= xmax; x++) {
        ...
        iterate_bead_coll_sol(pos, vel, pos_sol, vel_sol, sigma_sb2, box,
                              counter, counter_sol, event_queue, times,
                              times_sol, cells_sol, icell, i, step_time);
      }
    }
  }
}

void add_events_for_one_sol_coll_bead(
    const std::vector<Vec3> &pos_sol, const std::vector<Vec3> &vel_sol,
    const std::vector<Vec3> &pos, const std::vector<Vec3> &vel,
    const double sigma_sb2, const Box &box,
    const std::vector<uint64_t> &counter_sol,
    const std::vector<uint64_t> &counter, EventQueue &event_queue,
    const std::vector<double> &times_sol, const std::vector<double> &times,
    const Cells &cells_bead, const Cells &cells_sol, unsigned int i_sol,
    double step_time) {
  unsigned int zmin, zmax, ymin, ymax, xmin, xmax;
  ...
  for (unsigned int z = zmin; z <= zmax; z++) {
    for (unsigned int y = ymin; y <= ymax; y++) {
      for (unsigned int x = xmin; x <= xmax; x++) {
        ...
        unsigned int icell = icell_x + icell_y * cells_sol.ncell +
                             icell_z * cells_sol.ncell * cells_sol.ncell;
        iterate_sol_coll_bead(pos_sol, vel_sol, pos, vel, sigma_sb2, box,
                              counter_sol, counter, event_queue, times_sol,
                              times, cells_bead, icell, i_sol, step_time);
      }
    }
  }
}
...
void add_events_for_all_sol(
    const std::vector<Vec3> &pos, const std::vector<Vec3> &vel,
    const std::vector<Vec3> &pos_sol, const std::vector<Vec3> &vel_sol,
    unsigned int nbeads, const double sigma_sb2, const Box &box,
    const std::vector<uint64_t> &counter,
    const std::vector<uint64_t> &counter_sol, EventQueue &event_queue,
    const std::vector<double> &times, const std::vector<double> &times_sol,
    const Cells &cells_bead, const Cells &cells_sol, double step_time) {
  assert(pos.size() == nbeads);
  for (unsigned int i = 0; i < nbeads; i++) {
    add_events_for_one_bead_coll_sol(
        pos, vel, pos_sol, vel_sol, sigma_sb2, box, counter, counter_sol,
        event_queue, times, times_sol, cells_bead, cells_sol, i, step_time);
  }
}
...
void if_cell_sol(const std::vector<Vec3> &pos_sol,
                 const std::vector<Vec3> &vel_sol, const Box &box,
                 const std::vector<uint64_t> &counter_sol,
                 EventQueue &event_queue, const std::vector<double> &times_sol,
                 unsigned int i, const Cells &cells, double step_time) {
  double dt;
  SolventCellEvent::Wall wall;
  unsigned int ixn, iyn, izn;

  if (t_until_cell(pos_sol[i].x, pos_sol[i].y, pos_sol[i].z, box, cells,
                   vel_sol[i].x, vel_sol[i].y, vel_sol[i].z, cells.cell_x[i],
                   cells.cell_y[i], cells.cell_z[i], dt, wall, ixn, iyn, izn) &&
      ((times_sol[i] + dt) <= step_time)) {
    SolventCellEvent ev{
        times_sol[i] + dt, i, counter_sol[i], ixn, iyn, izn, wall};
    LOG_DEBUG("queueing " << ev);
    event_queue.emplace(ev);
  }
}
...
void init_cell_sol_events(const std::vector<Vec3> &pos_sol,
                          const std::vector<Vec3> &vel_sol, unsigned int nsol,
                          const Box &box,
                          const std::vector<uint64_t> &counter_sol,
                          EventQueue &event_queue,
                          const std::vector<double> &times_sol,
                          const Cells &cells, double step_time) {
  for (unsigned int i = 0; i < nsol; i++) {
    if_cell_sol(pos_sol, vel_sol, box, counter_sol, event_queue, times_sol, i,
                cells, step_time);
  }
}
...
void add_events_for_bead_coll_sol_after_crossing(
    const std::vector<Vec3> &pos, const std::vector<Vec3> &vel,
    const std::vector<Vec3> &pos_sol, const std::vector<Vec3> &vel_sol,
    const double sigma_sb2, const Box &box,
    const std::vector<uint64_t> &counter,
    ...
    const std::vector<double> &times, const std::vector<double> &times_sol,
    const Cells &cells_bead, const Cells &cells_sol, unsigned int i,
    BeadCellEvent::Wall wall, double step_time) {
  unsigned int zmin, zmax, ymin, ymax, xmin, xmax;
  walls_of_cell(zmin, zmax, ymin, ymax, xmin, xmax, cells_bead, i);
...
void add_events_for_sol_coll_bead_after_crossing(
    const std::vector<Vec3> &pos_sol, const std::vector<Vec3> &vel_sol,
    const std::vector<Vec3> &pos, const std::vector<Vec3> &vel,
    const double sigma_sb2, const Box &box,
    const std::vector<uint64_t> &counter_sol,
    ...
    const std::vector<double> &times_sol, const std::vector<double> &times,
    const Cells &cells_bead, const Cells &cells_sol, unsigned int i_sol,
    SolventCellEvent::Wall wall, double step_time) {
  unsigned int zmin, zmax, ymin, ymax, xmin, xmax;
  walls_of_cell(zmin, zmax, ymin, ymax, xmin, xmax, cells_sol, i_sol);
...
bool check_overlap(const unsigned int i, const std::vector<Vec3> &pos,
                   ...
                   const std::vector<Vec3> &pos_sol,
                   const std::vector<double> &times,
                   const std::vector<double> &times_sol, const double sigma_sb2,
                   const Box &box) {
  const unsigned int nbeads = pos.size();
...
bool final_vel(const std::vector<Vec3> &pos, std::vector<Vec3> &vel,
               const std::vector<Vec3> &pos_sol, std::vector<Vec3> &vel_sol,
               const double sigma2, unsigned int i, unsigned int j,
               const Box &box, const std::optional<double> &dU, const double m,
               const double m_sol) {
  ...
  return v_after_coll(dx, dy, dz, sigma2, vel[i].x, vel[i].y, vel[i].z,
                      vel_sol[j].x, vel_sol[j].y, vel_sol[j].z, dU, m, m_sol);
}

double total_k_E(const std::vector<Vec3> &vel, const std::vector<Vec3> &vel_sol,
                 const double m, const double m_sol) {
...
double compute_hamiltonian(const std::vector<Vec3> &vel,
                           const std::vector<Vec3> &vel_sol, const double m,
                           const double m_sol, UpdateConfig &update_config) {
...
bool process_event(const MinNearestEvent &ev, System &sys, const Param &p,
                   ...
                   Cells &cells_sol, UpdateConfig &update_config,
                   UpdateConfigWriter &, double step_time) {
  ...
  assert(check_overlap(ev.i, sys.pos, sys.vel, sys.pos, sys.times, sys.times,
                       p.sigma_bb2, box));
  assert(check_overlap(ev.j, sys.pos, sys.vel, sys.pos, sys.times, sys.times,
                       p.sigma_bb2, box));

  final_vel(sys.pos, sys.vel, sys.pos, sys.vel, p.near_min2, ev.i, ev.j, box,
            {}, p.m, p.m);
  ...
  add_events_for_one_bead(sys.pos, sys.vel, p.rh2, p.rc2, box, sys.counter,
                          ...
                          p.max_nbonds);
  add_events_for_one_bead_coll_sol(
      sys.pos, sys.vel, sys.pos_sol, sys.vel_sol, p.sigma_sb2, box, sys.counter,
      sys.counter_sol, event_queue, sys.times, sys.times_sol, cells_bead,
      cells_sol, ev.i, step_time);
  add_events_for_one_bead_coll_sol(
      sys.pos, sys.vel, sys.pos_sol, sys.vel_sol, p.sigma_sb2, box, sys.counter,
      sys.counter_sol, event_queue, sys.times, sys.times_sol, cells_bead,
      cells_sol, ev.j, step_time);
...
bool process_event(const MaxNearestEvent &ev, System &sys, const Param &p,
  ...
  final_vel(sys.pos, sys.vel, sys.pos, sys.vel, p.near_max2, ev.i, ev.j, box,
            {}, p.m, p.m);
...
bool process_event(const MinNNearestEvent &ev, System &sys, const Param &p,
  ...
  final_vel(sys.pos, sys.vel, sys.pos, sys.vel, p.nnear_min2, ev.i, ev.j, box,
            {}, p.m, p.m);
...
bool process_event(const MaxNNearestEvent &ev, System &sys, const Param &p,
  ...
  final_vel(sys.pos, sys.vel, sys.pos, sys.vel, p.nnear_max2, ev.i, ev.j, box,
            {}, p.m, p.m);
...
bool process_event(const MinNonlocalInnerEvent &ev, System &sys, const Param &p,
  ...
  final_vel(sys.pos, sys.vel, sys.pos, sys.vel, p.rh2, ev.i, ev.j, box, {}, p.m,
            p.m);
...
bool process_event(const MaxNonlocalInnerEvent &ev, System &sys, const Param &p,
                   const Box &box, EventQueue &event_queue, Cells &cells_bead,
                   Cells &cells_sol, UpdateConfig &update_config,
                   UpdateConfigWriter &update_config_writer, double step_time) {
  const unsigned int nbeads = sys.pos.size();
  ...
  if (final_vel(sys.pos, sys.vel, sys.pos, sys.vel, p.rc2, ev.i, ev.j, box,
                -p.eps, p.m, p.m)) {
    assert(update_config.non_bonded(t_bond_mask));
    update_config.flip_bond(t_bond_mask);
    assert(update_config.bonded(t_bond_mask));
...
bool process_event(const MaxNonlocalOuterEvent &ev, System &sys, const Param &p,
  ...
  if (final_vel(sys.pos, sys.vel, sys.pos, sys.vel, p.rc2, ev.i, ev.j, box, dU,
                p.m, p.m)) {
    assert(update_config.bonded(t_bond_mask));
    update_config.flip_bond(t_bond_mask);
    assert(update_config.non_bonded(t_bond_mask));
...
bool process_event(const BeadCollidesSolEvent &ev, System &sys, const Param &p,
                   const Box &box, EventQueue &event_queue, Cells &cells_bead,
                   Cells &cells_sol, UpdateConfig &update_config,
                   UpdateConfigWriter &, double step_time) {
  const unsigned int nbeads = sys.pos.size();
  ...
  assert(check_overlap(ev.i, sys.pos, sys.vel, sys.pos, sys.times, sys.times,
                       p.sigma_bb2, box));
  assert(check_overlap(ev.j, sys.pos, sys.vel, sys.pos_sol, sys.times,
                       sys.times_sol, p.sigma_sb2, box));

  final_vel(sys.pos, sys.vel, sys.pos_sol, sys.vel_sol, p.sigma_sb2, ev.i, ev.j,
            box, {}, p.m, p.m_sol);
  ...
  add_events_for_one_bead(sys.pos, sys.vel, p.rh2, p.rc2, box, sys.counter,
                          ...
                          p.transient_bonds, p.permanent_bonds, update_config,
                          p.max_nbonds);
  add_events_for_one_sol_coll_bead(
      sys.pos_sol, sys.vel_sol, sys.pos, sys.vel, p.sigma_sb2, box,
      sys.counter_sol, sys.counter, event_queue, sys.times_sol, sys.times,
      cells_bead, cells_sol, ev.j, step_time);
  add_events_for_one_bead_coll_sol(
      sys.pos, sys.vel, sys.pos_sol, sys.vel_sol, p.sigma_sb2, box, sys.counter,
      sys.counter_sol, event_queue, sys.times, sys.times_sol, cells_bead,
      cells_sol, ev.i, step_time);
  ...
  if_cell(sys.pos, sys.vel, box, sys.counter, event_queue, sys.times, ev.i,
          cells_bead);
  if_cell_sol(sys.pos_sol, sys.vel_sol, box, sys.counter_sol, event_queue,
              sys.times_sol, ev.j, cells_sol, step_time);
...
bool process_event(const BeadCellEvent &ev, System &sys, const Param &p,
  ...
  add_events_for_bead_coll_sol_after_crossing(
      sys.pos, sys.vel, sys.pos_sol, sys.vel_sol, p.sigma_sb2, box, sys.counter,
      sys.counter_sol, event_queue, sys.times, sys.times_sol, cells_bead,
      cells_sol, ev.i, ev.wall, step_time);
...
bool process_event(const SolventCellEvent &ev, System &sys, const Param &p,
                   const Box &box, EventQueue &event_queue, Cells &cells_bead,
                   Cells &cells_sol, UpdateConfig &, UpdateConfigWriter &,
                   double step_time) {
  ...
  add_events_for_sol_coll_bead_after_crossing(
      sys.pos_sol, sys.vel_sol, sys.pos, sys.vel, p.sigma_sb2, box,
      sys.counter_sol, sys.counter, event_queue, sys.times_sol, sys.times,
      cells_bead, cells_sol, ev.i, ev.wall, step_time);

  if_cell_sol(sys.pos_sol, sys.vel_sol, box, sys.counter_sol, event_queue,
              sys.times_sol, ev.i, cells_sol, step_time);
