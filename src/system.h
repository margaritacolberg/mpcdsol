...
struct System {
    ...
    System(unsigned int nbeads, unsigned int nsol) : pos(nbeads), pos_sol(nsol), vel(nbeads), vel_sol(nsol), times(nbeads), times_sol(nsol), counter(nbeads), counter_sol(nsol) {}
};
