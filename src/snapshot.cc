...
void write_snapshot(const std::string snapshot_name, const std::vector<Vec3>& pos, Random& mt, UpdateConfig& update_config) {
    H5::H5File file(snapshot_name, H5F_ACC_TRUNC);

    {
        const hsize_t mem_dims[2] = {pos.size(), 3};
        H5::DataSpace mem_space(2, mem_dims);
        H5::DataSet dataset_pos = file.createDataSet("pos", H5::PredType::NATIVE_DOUBLE, mem_space);
    ...
    H5::Attribute attr_config = file.createAttribute("config", H5::PredType::NATIVE_UINT64, H5S_SCALAR);
    attr_config.write(H5::PredType::NATIVE_UINT64, &update_config.config);
}

void read_snapshot(const std::string snapshot_name, std::vector<Vec3>& pos, Random& mt, UpdateConfig& update_config) {
...
void read_input(const std::string input_name, std::vector<Vec3>& pos) {
    ...
    dataset_pos.read(pos.data(), H5::PredType::NATIVE_DOUBLE, mem_space, file_space);
}
