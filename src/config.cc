...
unsigned int NonlocalBonds::get_nbonds() const {
    return ij_.size();
}

// write the transient or permanent bead indices to the output file
void NonlocalBonds::write_hdf5(H5::H5Object& obj, const std::string& name) const {
    const hsize_t dims[2] = {ij_.size(), 2};
    H5::DataSpace space(2, dims);
    H5::Attribute attr = obj.createAttribute(name, H5::PredType::NATIVE_UINT, space);
    if (!ij_.empty()) {
...
UpdateConfigWriter::UpdateConfigWriter(H5::H5File& file) {
    ...
    dataset_int = group.createDataSet("int", H5::PredType::NATIVE_UINT64, file_space, dcpl);
    dataset_time = group.createDataSet("time", H5::PredType::NATIVE_DOUBLE, file_space, dcpl);
}
...
void UpdateConfigWriter::append() {
    ...
    dataset_int.write(&config_int[0], H5::PredType::NATIVE_UINT64, mem_space, file_space);
    dataset_time.write(&config_time[0], H5::PredType::NATIVE_DOUBLE, mem_space, file_space);
}
