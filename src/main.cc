...
void initialize_pos(System &sys, Random &mt, const Param &p, const Box &box,
                       UpdateConfig &update_config, std::optional<std::string> input_name, std::optional<std::string> snapshot_name) {
    ...
    init_pos_sol(sys.pos, sys.pos_sol, box, mt, p);
}

void initialize_system(System &sys, Random &mt, const Param &p, const Box &box,
                       UpdateConfig &update_config, Cells &cells_bead, Cells &cells_sol,
                       EventQueue &event_queue) {
    if (!check_local_dist(sys.pos, box, p.near_min2, p.near_max2, p.nnear_min2, p.nnear_max2)) {
    ...
    if (!(cells_sol.ncell >= 4)) {
        throw std::invalid_argument("sol ncell must be at least 4");
    }

    if (!(cells_sol.lcell >= p.sigma_sb)) {
        throw std::invalid_argument("sol lcell must be at least sigma_sb");
    }

    init_vel(sys.vel, mt, p.temp, p.m);

    init_vel_sol(sys.vel_sol, mt, p.temp, p.m_sol);

    // split the box into smaller cells, and store the
    // beads and solvent particles in each cell
    init_cells(sys.pos, box, cells_bead);
    init_cells(sys.pos_sol, box, cells_sol);

    // fill priority queue with cell crossings of all particles
    init_cell_events(sys.pos, sys.vel, p.nbeads, box, sys.counter, event_queue, sys.times, cells_bead);
    init_cell_sol_events(sys.pos_sol, sys.vel_sol, p.nsol, box, sys.counter_sol, event_queue, sys.times_sol, cells_sol, p.del_t);

    // fill priority queue with nearest bond events between all particle
    // pairs
    init_nearest_bond_events(sys.pos, sys.vel, p.nbeads, box, sys.counter, event_queue, sys.times, p.near_min2, p.near_max2);

    // fill priority queue with next-nearest bond events between all
    // particle pairs
    init_nnearest_bond_events(sys.pos, sys.vel, p.nbeads, box, sys.counter, event_queue, sys.times, p.nnear_min2, p.nnear_max2);
    ...
    // queue)
    add_events_for_all_beads(sys.pos, sys.vel, p.nbeads, p.rh2, p.rc2, box, sys.counter, event_queue, sys.times, cells_bead, p.transient_bonds, p.permanent_bonds, update_config, p.max_nbonds);
    add_events_for_all_sol(sys.pos, sys.vel, sys.pos_sol, sys.vel_sol, p.nbeads, p.sigma_sb2, box, sys.counter, sys.counter_sol, event_queue, sys.times, sys.times_sol, cells_bead, cells_sol, p.del_t);
}

void run_step(System &sys, const Param &p, const Box &box, UpdateConfig
        &update_config, UpdateConfigWriter &update_config_writer, uint64_t
        countevents, uint64_t validevents, double wall_time, Cells &cells_bead,
        Cells &cells_sol, EventQueue &event_queue, double step_time) {
    ...
    while (!event_queue.empty()) {
        ...
        const double E_i = compute_hamiltonian(sys.vel, sys.vel_sol, p.m, p.m_sol, update_config);

        // process collision or cell crossing event
        if (std::visit([&](auto&& ev) {
                    wall_time = ev.t;
                    LOG_DEBUG("wall time " << wall_time);
                    return process_event(ev, sys, p, box, event_queue, cells_bead, cells_sol, update_config, update_config_writer, step_time);
                    }, event)) validevents++;

        const double E_f = compute_hamiltonian(sys.vel, sys.vel_sol, p.m, p.m_sol, update_config);
        if (E_i != 0.0) {
            const double E_diff = std::abs(1 - (E_f / E_i));
    ...
    for (unsigned int i = 0; i < p.nbeads; i++) {
        ...
        assert(check_overlap(i, sys.pos, sys.vel, sys.pos, sys.times, sys.times, p.sigma_bb2, box));
    }

    // check bond distances of nearest and next-nearest beads
    assert(check_local_dist(sys.pos, box, p.near_min2, p.near_max2, p.nnear_min2, p.nnear_max2));

    // check bond distances of nonlocal beads
    assert(check_nonlocal_dist(sys.pos, box, p.rc2, p.rh2, p.permanent_bonds));
}

void solvent_mpcd(System &sys, const Param &p, const Box &box, Cells &cells_mpcd, Random& mt, double step_time) {
    // shift grid of cells to preserve Galilean invariance
    shift_grid_amount(sys.pos_sol, box, cells_mpcd, mt);

    // solvent streaming step
    for (unsigned int i = 0; i < p.nsol; i++) {
        update_pos(sys.pos_sol[i], sys.vel_sol[i], sys.times_sol[i], step_time);
        assert(check_overlap(i, sys.pos, sys.vel, sys.pos_sol, sys.times, sys.times_sol, p.sigma_sb2, box));
    }

    // solvent collision step
    collide_sol(sys.vel_sol, mt, cells_mpcd, p.angle);
...
// equilibrate initial configuration, before carrying out bond forming or breaking events
void run_trajectory_eq(System &sys, const Param &p, const Box &box, UpdateConfig &update_config, UpdateConfigWriter &update_config_writer, Cells &cells_bead, Cells &cells_sol, Cells &cells_mpcd, EventQueue &event_queue, uint64_t countevents, uint64_t validevents, Random &mt, double wall_time) {
    ...
    for (unsigned int step = 0; step < p.nsteps; step++) {
        ...
        run_step(sys, p, box, update_config, update_config_writer, countevents, validevents, wall_time, cells_bead, cells_sol, event_queue, step_time);

        solvent_mpcd(sys, p, box, cells_mpcd, mt, step_time);

        init_cells(sys.pos_sol, box, cells_sol);

        if ((step + 1) < p.nsteps) {
        double next_step = (step + 1) * p.del_t;

        init_cell_sol_events(sys.pos_sol, sys.vel_sol, p.nsol, box, sys.counter_sol, event_queue, sys.times_sol, cells_sol, next_step);
        add_events_for_all_sol(sys.pos, sys.vel, sys.pos_sol, sys.vel_sol, p.nbeads, p.sigma_sb2, box, sys.counter, sys.counter_sol, event_queue, sys.times, sys.times_sol, cells_bead, cells_sol, next_step);
        }
...
void run_trajectory(System &sys, const Param &p, const Box &box, UpdateConfig &update_config, UpdateConfigWriter &update_config_writer, Cells &cells_bead, Cells &cells_sol, Cells &cells_mpcd, EventQueue &event_queue, PosWriter &pos_writer, VelWriter &vel_writer, ConfigWriter &config_writer, std::set<Config> &store_config, ConfigInt &store_config_int, std::vector<uint64_t> &config_sum, uint64_t countevents, uint64_t validevents, Random &mt, double wall_time, unsigned int iter, const H5::DataSpace &mem_space, H5::DataSpace &file_space, H5::DataSet &dataset_pos) {
    for (unsigned int step = 0; step < p.nsteps; step++) {
        std::cout << "step = " << step << std::endl;
        ...
        dataset_pos.write(&sys.pos[0], H5::PredType::NATIVE_DOUBLE, mem_space, file_space);
        ...
        if (step % p.write_step == 0) {
            ...
            // to file
            if (update_config.config == 1 && store_config.insert(update_config.config).second) {
...
int main(int argc, char* argv[]) {
    ...
    desc.add_options()
        ...
        ("output-file", po::value<std::string>()->required(), "h5 output")
        ("input-file", po::value<std::string>(), "h5 input")
        ("snapshot-file", po::value<std::string>(), "h5 snapshot")
        ;
    ...
    try {
        ...
        if (vm.count("help")) {
            std::cout << "Usage: hardspheres [options]... json-file output-file" << std::endl;
    ...
    H5::Attribute attr_sigma_bb = file.createAttribute("sigma_bb", H5::PredType::NATIVE_DOUBLE, H5S_SCALAR);
    attr_sigma_bb.write(H5::PredType::NATIVE_DOUBLE, &p.sigma_bb);
    H5::Attribute attr_length = file.createAttribute("length", H5::PredType::NATIVE_DOUBLE, H5S_SCALAR);
    attr_length.write(H5::PredType::NATIVE_DOUBLE, &box.l);
    H5::Attribute attr_nsteps = file.createAttribute("nsteps", H5::PredType::NATIVE_UINT, H5S_SCALAR);
    ...
    H5::DataSet dataset_pos = file.createDataSet("pos", H5::PredType::NATIVE_FLOAT, file_space);
    ...
    for (unsigned int iter = 0; iter < p.total_iter; iter++) {
        // reset bead clocks and counters
        for (unsigned int i = 0; i < p.nbeads; i++) {
            sys.times[i] = 0.0;
            sys.counter[i] = 0.0;
        }

        // reset solvent clocks and counters
        for (unsigned int i = 0; i < p.nsol; i++) {
            sys.times_sol[i] = 0.0;
            sys.counter_sol[i] = 0.0;
        }

        double wall_time = 0.0;

        Cells cells_bead{p.ncell, p.length / p.ncell};
        Cells cells_sol{p.ncell, p.length / p.ncell};
        Cells cells_mpcd{p.ncell_mpcd, p.length / p.ncell_mpcd};
        ...
        initialize_pos(sys, mt, p_eq, box, update_config_eq, input_name, snapshot_name);
        initialize_system(sys, mt, p_eq, box, update_config_eq, cells_bead, cells_sol, event_queue_eq);

        run_trajectory_eq(sys, p_eq, box, update_config_eq, update_config_writer, cells_bead, cells_sol, cells_mpcd, event_queue_eq, countevents, validevents, mt, wall_time);
        ...
        init_update_config(sys.pos, update_config, box, p.rc2, p.transient_bonds);
        initialize_system(sys, mt, p, box, update_config, cells_bead, cells_sol, event_queue);

        run_trajectory(sys, p, box, update_config, update_config_writer, cells_bead, cells_sol, cells_mpcd, event_queue, pos_writer, vel_writer, config_writer, store_config, store_config_int, config_sum, countevents, validevents, mt, wall_time, iter, mem_space, file_space, dataset_pos);
    ...
    H5::Attribute attr_countevents = file.createAttribute("countevents", H5::PredType::NATIVE_UINT64, H5S_SCALAR);
    attr_countevents.write(H5::PredType::NATIVE_UINT64, &countevents);
    H5::Attribute attr_validevents = file.createAttribute("validevents", H5::PredType::NATIVE_UINT64, H5S_SCALAR);
