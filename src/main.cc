...
void initialize_system(System &sys, Random &mt, const Param &p, const Box &box,
                       ...
                       Cells &cells_sol, EventQueue &event_queue) {
  ...
  init_cell_sol_events(sys.pos_sol, sys.vel_sol, p.nsol, box, sys.counter_sol,
                       event_queue, sys.times_sol, cells_sol, p.del_t);
  ...
  add_events_for_all_beads(sys.pos, sys.vel, p.nbeads, p.rh2, p.rc2, box,
                           ...
                           p.max_nbonds);
  add_events_for_all_sol(sys.pos, sys.vel, sys.pos_sol, sys.vel_sol, p.nbeads,
                         p.sigma_sb2, box, sys.counter, sys.counter_sol,
                         event_queue, sys.times, sys.times_sol, cells_bead,
                         cells_sol, p.del_t);
}
...
void run_step(System &sys, const Param &p, const Box &box,
  ...
  while (!event_queue.empty()) {
    ...
    const double E_i =
        compute_hamiltonian(sys.vel, sys.vel_sol, p.m, p.m_sol, update_config);
    ...
    if (std::visit(
            [&](auto &&ev) {
              ...
              return process_event(ev, sys, p, box, event_queue, cells_bead,
                                   cells_sol, update_config,
                                   update_config_writer, step_time);
            },
            event))
      validevents++;
  ...
  for (unsigned int i = 0; i < p.nbeads; i++) {
    ...
    assert(check_overlap(i, sys.pos, sys.vel, sys.pos, sys.times, sys.times,
                         p.sigma_bb2, box));
  }
...
void solvent_mpcd(System &sys, const Param &p, const Box &box,
                  Cells &cells_mpcd, Random &mt, double step_time) {
  ...
  for (unsigned int i = 0; i < p.nsol; i++) {
    ...
    assert(check_overlap(i, sys.pos, sys.vel, sys.pos_sol, sys.times,
...
void run_trajectory_eq(System &sys, const Param &p, const Box &box,
                       ...
                       Cells &cells_bead, Cells &cells_sol, Cells &cells_mpcd,
                       EventQueue &event_queue, uint64_t countevents,
  ...
  for (unsigned int step = 0; step < p.nsteps; step++) {
    ...
    run_step(sys, p, box, update_config, update_config_writer, countevents,
             validevents, wall_time, cells_bead, cells_sol, event_queue,
             step_time);
    ...
    if ((step + 1) < p.nsteps) {
      ...
      init_cell_sol_events(sys.pos_sol, sys.vel_sol, p.nsol, box,
                           sys.counter_sol, event_queue, sys.times_sol,
                           cells_sol, next_step);
      add_events_for_all_sol(sys.pos, sys.vel, sys.pos_sol, sys.vel_sol,
                             p.nbeads, p.sigma_sb2, box, sys.counter,
                             sys.counter_sol, event_queue, sys.times,
                             sys.times_sol, cells_bead, cells_sol, next_step);
    }
...
void run_trajectory(System &sys, const Param &p, const Box &box,
                    ...
                    Cells &cells_sol, Cells &cells_mpcd,
...
int main(int argc, char *argv[]) {
  ...
  desc.add_options()("help,h", "produce help message")(
      "json-file", po::value<std::string>()->required(),
      "json")("output-file", po::value<std::string>()->required(),
              "h5 output")("input-file", po::value<std::string>(), "h5 input")(
  ...
  for (unsigned int iter = 0; iter < p.total_iter; iter++) {
    ...
    initialize_system(sys, mt, p_eq, box, update_config_eq, cells_bead,
                      cells_sol, event_queue_eq);

    run_trajectory_eq(sys, p_eq, box, update_config_eq, update_config_writer,
                      cells_bead, cells_sol, cells_mpcd, event_queue_eq,
                      countevents, validevents, mt, wall_time);
    ...
    init_update_config(sys.pos, update_config, box, p.rc2, p.transient_bonds);
    initialize_system(sys, mt, p, box, update_config, cells_bead, cells_sol,
                      event_queue);

    run_trajectory(sys, p, box, update_config, update_config_writer, cells_bead,
                   cells_sol, cells_mpcd, event_queue, pos_writer, vel_writer,
                   config_writer, store_config, store_config_int, config_sum,
                   countevents, validevents, mt, wall_time, iter, mem_space,
                   file_space, dataset_pos);
