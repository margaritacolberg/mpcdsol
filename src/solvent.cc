...
void shift_grid(double dx, double dy, double dz,
                const std::vector<Vec3> &pos_sol, const Box &box,
                Cells &cells) {
  const unsigned int nsol = pos_sol.size();
  ...
  for (unsigned int j = 0; j < nsol; j++) {
    ...
    LOG_DEBUG("particle " << j << " at " << pos_sol[j].x << ", " << pos_sol[j].y
                          << ", " << pos_sol[j].z << " in cell " << icell_x
                          << ", " << icell_y << ", " << icell_z);
...
void shift_grid_amount(const std::vector<Vec3> &pos_sol, const Box &box,
                       Cells &cells, Random &mt) {
...
void collide_sol(std::vector<Vec3> &vel_sol, Random &mt, Cells &cells,
                 unsigned int angle) {
