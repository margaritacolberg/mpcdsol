...
#ifndef MPCD_CONFIG_H
#define MPCD_CONFIG_H
...
struct UpdateConfig {
    ...
    bool bonded(Config mask) const {
        return config & mask;
    }

    bool non_bonded(Config mask) const {
        return ~config & mask;
    }

    void flip_bond(Config mask) {
        config ^= mask;
    }

    unsigned int count_bonds() const {
        return std::popcount(config);
    }
